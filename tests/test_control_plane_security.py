...
def make_client(tmp_path: Path) -> tuple[TestClient, Path]:
    ...
    return TestClient(create_app(str(database_path))), database_path
...
def test_enrollment_is_one_time_expiring_and_device_bound(tmp_path: Path) -> None:
    ...
    code_response = client.post("/api/v1/enrollment-codes", json={"device_id": "device-a"})
    code = code_response.json()["code"]
    ...
    expired = client.post("/api/v1/enrollment-codes", json={"device_id": "device-a", "expires_in_seconds": 1}).json()["code"]
...
def test_plan_approval_is_persisted_and_bound(tmp_path: Path) -> None:
    client, database_path = make_client(tmp_path)
    create = client.post("/api/v1/devices/device-a/plans", json={"desired_commit": "commit-a", "current_hash": "hash-a"})
    assert create.status_code == 200
    plan_id = create.json()["id"]
    assert client.get("/api/v1/devices/device-a/plans").json()[0]["id"] == plan_id
    assert client.post(f"/api/v1/devices/device-a/plans/{plan_id}/approve", json={"current_hash": "wrong", "desired_commit": "commit-a"}).status_code == 409
    assert client.post(f"/api/v1/devices/device-a/plans/{plan_id}/approve", json={"current_hash": "hash-a", "desired_commit": "wrong"}).status_code == 409
    assert client.post(f"/api/v1/devices/device-a/plans/{plan_id}/approve", json={"current_hash": "hash-a", "desired_commit": "commit-a"}).status_code == 200
    connection = sqlite3.connect(database_path)
    ...
    connection.commit()
    connection.close()
    assert client.post(f"/api/v1/devices/device-a/plans/{plan_id}/approve", json={"current_hash": "hash-a", "desired_commit": "commit-a"}).status_code == 409
